...
describe("Unit test for proving and checking correctness of ZKPs on linking info", () => {
    ...
    it("should accept a correctly formed linkInfo (using serialization in between)", () => {
        const researcher: Researcher = new Researcher(studyID1);
        const answer: Answer = student3.tagAnswer(questIDs[0], [studyID1], {
            a: "st3"
        });
        const serializedAnswer: string = answer.serialize();
        ...
        const recoveredAnswer: Answer = Answer.deserialize(serializedAnswer);

        expect(
            researcher.checkAnswerCorrectness(
                recoveredAnswer,
                issuer.publicKey,
                nym
            )
