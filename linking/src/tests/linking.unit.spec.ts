...
"use strict";
import { Answer, LinkInfo, Researcher, Student, setup, mcl } from "../linking";
...
function compareAnswers(ans1: Answer, ans2: Answer) {
    ...
    for (const studyID of Object.keys(ans1.linkInfos)) {
        if (
            ...
            ans1.linkInfos[studyID].serialize() !==
                ans2.linkInfos[studyID].serialize() ||
            !(ans2.linkInfos[studyID].s instanceof mcl.G2) ||
            !(ans2.linkInfos[studyID].t instanceof mcl.GT)
        )
            return false;
    }
...
describe("Integration tests for linking", () => {
    ...
    it("should not be able to link without the linking token", () => {
        const researcher: Researcher = new Researcher(studyIDs[0]);
        ...
        expect(checkLinking(researcher.links, 0)).to.equal(true);
        expect(Object.values(researcher.links).length).to.equal(0);
    });

    it("should not be able to link with the wrong linking token", () => {
        const researcher: Researcher = new Researcher(studyIDs[0]);
    ...
    it("should correctly serialize and deserialize an Answer object", () => {
        const ans: Answer = student1.tagAnswer(questIDs[0], studyIDs, {});

        const deserializedAns: Answer = Answer.deserialize(ans.serialize());

        expect(compareAnswers(ans, deserializedAns)).to.equal(true);
    });
});
